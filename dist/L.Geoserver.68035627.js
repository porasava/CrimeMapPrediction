  ...
  wmsImage: function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var wmsLayerURL = `${that.baseLayerUrl}/wms?service=WMS&version=1.1.0&request=GetMap&\
layers=${otherLayers}&\
styles=${otherStyle}&\
cql_filter=${otherCqlFilter}&\
bbox=${(bboxX1 + bboxX2) * 0.5 - maxValue - bufferBbox},${(bboxY1 + bboxY2) * 0.5 - maxValue - bufferBbox},${(bboxX1 + bboxX2) * 0.5 + maxValue + bufferBbox},${(bboxY1 + bboxY2) * 0.5 + maxValue + bufferBbox}&\
